...
router.post('/parsing/book', clientIp, isLoggedIn, async (req, res, next) => {
    try {
        ...
        winston.log('info', `[ADMIN][${req.clientIp}|${user_email}]  데이터 파싱`);
        ...
        if (user_email != process.env.ADMIN) {
            ...
            // json 데이터 불러오기
            const comicData = fs.readFileSync(path.join(__dirname, '../data/comicdata.json'), 'utf8');
            const romanceData = fs.readFileSync(path.join(__dirname, '../data/romancedata.json'), 'utf8');
            const fantasyData = fs.readFileSync(path.join(__dirname, '../data/fantasydata.json'), 'utf8');
        ...
        winston.log('error', `[ADMIN][${req.clientIp}|${req.body.email}] 파싱 Exception`);
